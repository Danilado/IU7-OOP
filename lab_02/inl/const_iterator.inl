...
template <typename T> T *ConstIterator<T>::getPtr() const {
  std::shared_ptr<T[]> res = ptr.lock();
  return res.get() + index;
}

template <typename T>
void ConstIterator<T>::checkExpired(const size_t line, string funcname) const {
  if (ptr.expired())
...
template <typename T>
void ConstIterator<T>::checkBounds(const size_t line, string funcname,
...
template <typename T> ConstIterator<T>::~ConstIterator() = default;
...
template <typename T> ConstIterator<T>::operator bool() const {
